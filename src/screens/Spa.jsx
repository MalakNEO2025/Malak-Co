...
import { VData as D } from '../data/data';

const { Button, Badge, Chip } = window.NEOLIFEDesignSystem_08f245;
...
const days = (() => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    out.push({ key: i, dow: i === 0 ? 'Today' : dn[d.getDay()], num: d.getDate() });
  }
  return out;
})();

export function Spa({ go, openExp }) {
      ...
      <div style={{ padding: '12px 30px 0', display: 'flex', flexDirection: 'column', gap: 14 }}>
        {items.map((x) => (
          <button key={x.id} onClick={() => openExp(x.id)} style={{ textAlign: 'left', cursor: 'pointer', display: 'flex', gap: 16, padding: 16, borderRadius: 18,
            ...
            <span style={{ width: 56, height: 56, flexShrink: 0, borderRadius: 14, background: 'var(--brand-soft)', color: 'var(--brand)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>{I[x.icon]({ size: 26 })}</span>
...
export function Booking({ go, id, addBooking }) {
...
export function BookingConfirmed({ go, lastBooking }) {
  const b = lastBooking || {};
