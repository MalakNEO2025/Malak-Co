/* Villa Igiea app — shared UI helpers. Exposes window.VUI. */
(function () {
  const I = window.VIcons;
  const fmt = (n) => '$' + Number(n).toFixed(0);

  // Screen scaffold: scrollable column with bottom room for nav
  ...
  // Sticky top bar with optional back chevron + right slot
  ...
  // Quantity stepper (− n +)
  function Qty({ value, onAdd, onRemove, small }) {
    const s = small ? 30 : 36;
    const btn = {
  ...
  // A round pill icon button (e.g. add to cart)
  function IconPill({ icon, onClick, label, brand = true }) {
    return (
  ...
  // Decorative photographic frame using the bar hero asset, tintable
  window.VUI = { Screen, TopBar, SectionTitle, Qty, IconPill, fmt };
})();
